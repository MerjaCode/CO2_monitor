...
void setup() {
  Serial.begin(9600);
  Serial.println("\n--- BOOT ---");
  Serial.println("file = SPKFuN_RV1805_TrueSleep_v33.ino");
  
  pinMode(WAKE_BUTTON, INPUT_PULLUP);
  
  // Use ext0 for the BOOT button (GPIO 0)
  ...
  delay(500); // Debounce delay for the power rail and button
...
void runCaptureCycle() {
  ...
  for (int i = 0; i < BURST_DURATION; i++) {
    ...
    if (currentCO2 > 0) {
      ...
      if (sdReady) {
        ...
        if (d) {
          d.printf("%s,%d,%d,%.2f\n", rtc.stringTime(), i, currentCO2, lipo.getVoltage());
          d.close();
        }
      }
    }
    ...
    display.printf("BAT: %.2fV (%.0f%%)", lipo.getVoltage(), lipo.getSOC());
  ...
  // 3. SHUTDOWN
  Serial.println("Closing SD and Cutting Power...");
  SD.end();
  SPI.end();
  ...
  // Power rail LOW (No rtc_hold possible on Pin 5)
